(function (e) {
  ...
  e.goMapBase = {
    defaults: {
      ...
      markers: [],
      overlays: [],
      polyline: { color: '#FF0000', opacity: 1, weight: 2 },
      polygon: { color: '#FF0000', opacity: 1, weight: 2, fillColor: '#FF0000', fillOpacity: 0.2 },
      circle: { color: '#FF0000', opacity: 1, weight: 2, fillColor: '#FF0000', fillOpacity: 0.2 },
      rectangle: { color: '#FF0000', opacity: 1, weight: 2, fillColor: '#FF0000', fillOpacity: 0.2 },
      maptype: 'HYBRID',
    ...
    init: function (t, r) {
      ...
      if (i.address) this.geocode({ address: i.address, center: true });
        ...
        if (i.markers[0].address) this.geocode({ address: i.markers[0].address, center: true });
      ...
      if (i.addMarker == true || i.addMarker == 'multi') {
        google.maps.event.addListener(a.map, 'click', function (e) {
          var t = { position: e.latLng, draggable: true };
    ...
    geoMarker: function () {
      if (this.geoMarkers.length > 0 && !this.lockGeocode) {
        ...
        this.geocode({ address: e[0].address }, e[0]);
    ...
    setMap: function (e) {
      delete e.mapTypeId;
      if (e.address) {
        this.geocode({ address: e.address, center: true });
        delete e.address;
      ...
      if (e.mapTypeControlOptions && e.mapTypeControlOptions.position)
        e.mapTypeControlOptions.position = google.maps.ControlPosition[e.mapTypeControlOptions.position.toUpperCase()];
      if (e.mapTypeControlOptions && e.mapTypeControlOptions.style)
        e.mapTypeControlOptions.style = google.maps.MapTypeControlStyle[e.mapTypeControlOptions.style.toUpperCase()];
      if (e.navigationControlOptions && e.navigationControlOptions.position)
        e.navigationControlOptions.position =
          google.maps.ControlPosition[e.navigationControlOptions.position.toUpperCase()];
      if (e.navigationControlOptions && e.navigationControlOptions.style)
        e.navigationControlOptions.style =
          google.maps.NavigationControlStyle[e.navigationControlOptions.style.toUpperCase()];
    ...
    createListener: function (t, n, r) {
      ...
      else if ((t.type == 'marker' || t.type == 'info') && this.getMarkerCount() != this.getTmpMarkerCount())
        var s = this;
      setTimeout(function () {
      ...
      switch (e.type) {
        case 'polyline':
          if (e.coords.length > 0) {
            for (var n = 0, r = e.coords.length; n < r; n++)
              t.push(new google.maps.LatLng(e.coords[n].latitude, e.coords[n].longitude));
        ...
        case 'rectangle':
          t = new google.maps.Rectangle({
            ...
            bounds: new google.maps.LatLngBounds(
              new google.maps.LatLng(e.sw.latitude, e.sw.longitude),
              new google.maps.LatLng(e.ne.latitude, e.ne.longitude)
            ),
            strokeColor: e.color ? e.color : this.opts.circle.color,
            strokeOpacity: e.opacity ? e.opacity : this.opts.circle.opacity,
            strokeWeight: e.weight ? e.weight : this.opts.circle.weight,
    ...
    setOverlay: function (t, n, r) {
      ...
      if (r.coords && r.coords.length > 0) {
        var i = [];
        for (var s = 0, o = r.coords.length; s < o; s++)
          i.push(new google.maps.LatLng(r.coords[s].latitude, r.coords[s].longitude));
        r.path = i;
        delete r.coords;
      } else if (r.ne && r.sw) {
        r.bounds = new google.maps.LatLngBounds(
          new google.maps.LatLng(r.sw.latitude, r.sw.longitude),
          new google.maps.LatLng(r.ne.latitude, r.ne.longitude)
        );
        delete r.ne;
        delete r.sw;
    ...
    createMarker: function (e) {
      ...
      if (e.address && !e.geocode) {
        ...
        if (e.html) {
          if (!e.html.content && !e.html.ajax && !e.html.id) e.html = { content: e.html };
    ...
    setMarker: function (t, n) {
      ...
      if (n.address) {
        this.geocode({ address: n.address }, { markerId: r });
        delete n.address;
        delete n.latitude;
        delete n.longitude;
        delete n.position;
